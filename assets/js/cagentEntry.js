$(document).ready(function(){  
  ...
  $(".text-danger").remove();
        ...
        if(a_name == "") {
          $("#a_name").closest('.form-control form-control-sm').addClass('has-error');
          $("#a_name").after('<td class="text-danger text-center" colspan="11">Please enter agent name<br></td>');
        } else {
          $("#a_name").closest('.form-control form-control-sm').removeClass('has-error');
          $("#a_name").closest('.form-control form-control-sm').addClass('has-success');        
        }
        if(a_name) 
         {  
             $.ajax({  
                   url:"CeoController/addAgententry",   
                   method:"POST",  
          ...
          $(".form-control form-control-sm").removeClass('has-error').removeClass('has-success');
                      ...
                      if(response.success == true) {
                        setTimeout(function() {
                            window.location.href = "agentmanage";
                        }, 2000);
                        Swal.fire({
                          icon: 'success',
                          title: 'Good Job!',
